...
try
{
 ...
 if(isset($_POST['codigoenfe'])){ $codigoenfe = $_POST['codigoenfe'];}else{$codigoenfe = '';}
	...
	$revpafil = mysqli_num_rows($revpares); if ($revpafil>0) { } else  { header("Location: hcitasenfe.php?notpa=off"); }
	$conn ->next_result();
 $rev = "SELECT * FROM `enfermera` WHERE `idempleado`= '$codigoenfe';";
 $revres = $conn->query($rev);
 $revfil = mysqli_num_rows($revres); if ($revfil>0) { } else  { header("Location: hcitasenfe.php?notuser=null"); }
	$conn ->next_result();
...
<center>
<form action="hcitasenferes.php" method="post">
<h2>Datos <span>Personales</span></h2><br>
<?php while($datos = $verinforres->fetch_assoc() ) { ?>
	<input class="form-input" type="text" name="duiuser" readonly value="<?php echo $duiuser;?>">
	<input class="form-input" type="text" name="nameuser" readonly value="<?php echo $datos['nombre'];?>">
	<input class="form-input" type="text" name="apellidouser" readonly value="<?php echo $datos['apellido'];?>">
	<?php } ?><br><br>
<h2>Informacion <span>de Cita</span></h2><br>
 ...
 <input class="form-input" type="text" name="codigoenfe" value="<?php echo $codigoenfe;?>" readonly>
<br><br>
...
<?php
	...
	<a href="hcitasenfe.php"><input type="button" value="Atras" class="volver"></a>
